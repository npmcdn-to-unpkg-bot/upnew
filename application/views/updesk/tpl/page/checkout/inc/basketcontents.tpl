...
<form name="basket[{$basketindex}]" action="[{$oViewConf->getSelfActionLink()}]" method="post">
    ...
    <ul id="basket" class="minibasketitems  [{if $oViewConf->getActiveClassName() != 'order'}] basket-articles[{/if}]">
        [{* basket items *}]
  ...
  [{assign var="_product" value=$basketitem}]
                      <li>
                        ...
                        <div class="item-infos">
                                ...
                                <div class="thi">[{if $_product->getVarSelect()}]<span class="size">Size: [{$_product->getVarSelect()}]</span>[{/if}]</div>
                                <div class="fou"></div>
                        </div>
                        ...
                        <div class="price-right">
                            <div class="fir">[{oxprice price=$basketitem->getFTotalPrice() currency=$currency}]</div>
                            <div class="sec">[{* [{if $_product->getAmount() gte 2}][{$_product->getAmount()}] x à [{oxprice price=$basketitem->getUnitPrice() currency=$currency}][{/if}] *}]</div>
    ...
    [{block name="checkout_basketcontents_basketfunctions"}]
        [{if $editable}]
            <div id="basketFn" class="basket-functions">
                ...
                <button id="basketUpdate" class="btn btn-default btn-xs" type="submit" name="updateBtn"><i class="glyphicon glyphicon-refresh"></i> [{oxmultilang ident="UPDATE"}]</button>
            </div>
        [{/if}]
    [{/block}]
            ...
            [{if !$oxcmp_basket->getDiscounts()}]
                ...
                [{*
                    <tr>
                        ...
                        <td id="basketTotalProductsNetto">[{oxprice price=$oxcmp_basket->getProductsNetPrice() currency=$currency}]</td>
                    </tr> *}]
                [{/block}]
                ...
                [{*
                    [{foreach from=$oxcmp_basket->getProductVats() item=VATitem key=key}]
                        <tr>
                            <th>[{oxmultilang ident="PLUS_VAT"}]&nbsp;[{$key}][{oxmultilang ident="AMOUNT"}]</th>
            ...
            [{else}]
                ...
                [{block name="checkout_basketcontents_discounts"}]
                    [{foreach from=$oxcmp_basket->getDiscounts() item=oDiscount name=test_Discounts}]
                        <tr>
                            <th>
                                    <b>[{if $oDiscount->dDiscount < 0}][{oxmultilang ident="SURCHARGE"}][{else}][{oxmultilang ident="DISCOUNT"}]code[{/if}]&nbsp;</b>
                                [{$oDiscount->sDiscount}]
                            </th>
                            <td>
                ...
                [{if !$oxcmp_basket->isPriceViewModeNetto()}]
                    [{block name="checkout_basketcontents_totalnet"}]
                        <tr>
                                <th>[{oxmultilang ident="TOTAL_NET"}]</th>
                            <td id="basketTotalNetto">[{oxprice price=$oxcmp_basket->getProductsNetPrice() currency=$currency}]</td>
                        </tr>
                    [{/block}]
                [{/if}]
                                ...
                                [{foreach from=$oxcmp_basket->getVouchers() item=sVoucher key=key name=Voucher}]
                                    <tr class="couponData">
                                        <th><span><strong>[{oxmultilang ident="COUPON"}]</strong>&nbsp;([{oxmultilang ident="NUMBER_2"}] [{$sVoucher->sVoucherNr}])</span>
                                            [{if $editable }]
                                                <a href="[{$oViewConf->getSelfLink()}]&amp;cl=basket&amp;fnc=removeVoucher&amp;voucherId=[{$sVoucher->sVoucherId}]&amp;CustomError=basket" class="btn btn-default btn-xs" rel="nofollow"><span class="glyphicon glyphicon-remove"></span> [{oxmultilang ident="REMOVE"}]</a>
                                            [{/if}]
                                        </th>
            ...
            [{block name="checkout_basketcontents_delcosts"}]
                [{if $oxcmp_basket->getDelCostNet() }]
                    <tr>
                        ...
                        <td id="basketDeliveryNetto">[{oxprice price=$oxcmp_basket->getDelCostNet() currency=$currency}]</td>
                    </tr>
                    [{if $oxcmp_basket->getDelCostVat() }]
                        <tr>
                            ...
                            [{else}]
                                        <th>[{oxmultilang ident="PLUS_VAT"}] [{$oxcmp_basket->getDelCostVatPercent()}][{oxmultilang ident="AMOUNT"}]</th>
                                [{/if}]
                ...
                [{elseif $oxcmp_basket->getFDeliveryCosts() }]
                    <tr>
                        ...
                        <td id="basketDeliveryGross">[{oxprice price=$oxcmp_basket->getFDeliveryCosts() currency=$currency}]</td>
                    </tr>
                [{/if }]
            [{/block}]
                    ...
                    [{if $oxcmp_basket->getPayCostVat() }]
                        <tr>
                            [{if $oxcmp_basket->isProportionalCalculationOn() }]
                                <th>[{oxmultilang ident="BASKET_TOTAL_PLUS_PROPORTIONAL_VAT"}]:</th>
                            [{else}]
                                <th>[{ oxmultilang ident="PAGE_CHECKOUT_BASKETCONTENTS_PAYMENTTAX1" }] [{ $oxcmp_basket->getPayCostVatPercent() }] [{ oxmultilang ident="AMOUNT" }]</th>
                            [{/if}]
