...
( function ( $ ) {
                ...
                activator, {//targetEl, onSuccess, onError, additionalData 
                    ...
                    'onSuccess' : function(r) {
                        ...
                        if ( typeof WidgetsHandler !== 'undefined') {
                            WidgetsHandler.reloadWidget('oxwarticletocartselector');
                        } else {
    ...
    function variantSelectActionHandler( e ) {
        ...
        if ( obj.parents('.js-fnSubmit').length > 0 ) {
            ...
            console.log( "function variantSelectActionHandler" + $(selection).val());
            
            var formProduct = obj.parents( 'form.js-oxProductForm' );
            $( 'input[name="' + $(selection).attr('name') + '"]', formProduct ).val( obj.data('selection-id') );


            var formWidget = obj.closest('.tocartselector_container').find(".js-oxWidgetReload");
    ...
    function formSubmit() {
        ...
        if (!$("input[name='fnc']", this).val()) {
            if (($( "input[name=aid]", this ).val() == $( "input[name=parentid]", this ).val() )) {
                var formProduct = target.closest('.tocartselector_container').find("form.js-oxProductForm");
                var aSelectionInputs = $("input[name^=varselid]", formProduct);
                ...
                if (aSelectionInputs.length) {
                    ...
                    if ( jQuery.inArray( sHash, oxVariantSelections ) === -1 ) {
                        return oxArticleVariant.reload( $(target), $("#details_container"),  target.closest('.tocartselector_container'), aOptions);
                    }
                }
            }
            // reload: function ( activator, highlightTargets, contentTarget, aOptions ) {
