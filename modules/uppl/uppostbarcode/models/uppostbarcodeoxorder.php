 ...
 */
class UpPostBarcodeOxOrder extends UpPostBarcodeOxOrder_parent
{
    ...
    public function pdfHeader($oPdf)
    {
        ...
        $aSize = getimagesize($myConfig->getImageDir() . '/pdf-logo-wide.png');
        $iMargin = 195 - $aSize[0] * 0.2;
        $oPdf->setLink($oShop->oxshops__oxurl->value);
        $oPdf->image($myConfig->getImageDir() . 'pdf-logo-wide.png', $iMargin, 10, $aSize[0] * 0.2, $aSize[1] * 0.2, '', $oShop->oxshops__oxurl->value);
    ...
    {   
        ...
        $oPdf->text( $horizoffset, 59, $this->oxorder__oxbillcompany->getRawValue());
        $oPdf->text( $horizoffset, 63, $sSal);
        $oPdf->text( $horizoffset, 67, $this->oxorder__oxbillfname->getRawValue() . ' ' . $this->oxorder__oxbilllname->getRawValue());
      
        $oPdf->text( $horizoffset, 71, $this->oxorder__oxbillstreet->getRawValue() . ' ' . $this->oxorder__oxbillstreetnr->value);
        $oPdf->text( $horizoffset, 75, $this->oxorder__oxbillzip->value . ' ' . $this->oxorder__oxbillcity->getRawValue());
        $oPdf->text( $horizoffset, 79, $this->oxorder__oxbillcountry->getRawValue());
    }
    ...
    {   
        ...
        $oPdfBlock = new InvoicepdfBlock();
        $oPdf->setFont($oPdfBlock->getFont(), 'B', 8);
        $oPdf->text($horizoffset, 87, $this->translate('ORDER_OVERVIEW_PDF_DELIVERYADDRESS'));
        $oPdf->setFont($oPdfBlock->getFont(), '', 10);
        $oPdf->text($horizoffset, 91, $this->oxorder__oxdelcompany->getRawValue());
        $oPdf->text($horizoffset, 95, $sSal);
        $oPdf->text($horizoffset, 99, $this->oxorder__oxdellname->getRawValue() . ' ' . $this->oxorder__oxdelfname->getRawValue());
        $oPdf->text($horizoffset, 103, $this->oxorder__oxdelstreet->getRawValue() . ' ' . $this->oxorder__oxdelstreetnr->value);
        $oPdf->text($horizoffset, 107, $this->oxorder__oxdelzip->value . ' ' . $this->oxorder__oxdelcity->getRawValue());
        $oPdf->text($horizoffset, 111, $this->oxorder__oxdelcountry->getRawValue());
    }
    ...
    protected function _setOrderArticlesToPdf($oPdf, &$iStartPos, $blShowPrice = true)
    {
        ...
        foreach ($this->_oArticles as $key => $oOrderArt) {
            ...
            if ($oOrderArt->oxorderarticles__oxselvariant->value) {
                ...
                $vartext = " - ".$oOrderArt->oxorderarticles__oxselvariant->value;
            }
                $title = substr(strip_tags($this->_replaceExtendedChars($oOrderArt->oxorderarticles__oxtitle->getRawValue(), true)), 0, 58);
            $oPdf->text(45, $iStartPos, $title.$vartext );
    ...
    public function exportStandart($oPdf)
    {
        ...
        // shop information
        $oPdf->setFont($oPdfBlock->getFont(), 'B', 7);
        $oPdf->text(130, 50, $oShop->oxshops__oxcompany->getRawValue() . ' - ' . $oShop->oxshops__oxstreet->getRawValue() . ' - ' . $oShop->oxshops__oxzip->value . ' - ' . $oShop->oxshops__oxcity->getRawValue());
        $oPdf->line(130, 51, 196, 51);
        // billing address
        ...
        $oPdf->setFont($oPdfBlock->getFont(), '', 10);
        $oPdf->text(15, $iTop + 8, $sText);

        // shop VAT number
        if ($oShop->oxshops__oxvatnumber->value && 1 == 2) {
       ...
       // $oPdf->text(195 - $oPdf->getStringWidth($sText), $iTop + 8, $sText);
        ...
        //$oPdf->text(15, $iTop, $this->translate('ORDER_OVERVIEW_PDF_STANDART') . ' ' . $this->oxorder__oxordernr->value."-".$this->oxorder__oxbillnr->value );
        $oPdf->setFont($oPdfBlock->getFont(), 'B', 12);
        
        $pdftype = oxRegistry::getConfig()->getRequestParameter('pdftype');
        
        if($pdftype=='mahnung1'){
            $pdftitle =  $this->translate( 'MAHNUNG1' );
        }elseif($pdftype=='mahnung2'){
            $pdftitle =  $this->translate( 'MAHNUNG2' );
        }else{
             $pdftitle = $this->translate('ORDER_OVERVIEW_PDF_STANDART');
        }
        $this->_setPdfTitle(  $oPdf, $iTop, $pdftitle );
       ...
       // $oPdf->text(15, $iTop + 8, $this->translate('ORDER_OVERVIEW_PDF_ORDERSFROM') . $sOrderDate . $this->translate('ORDER_OVERVIEW_PDF_ORDERSAT') . $oShop->oxshops__oxurl->value);
        ...
        /* Mahnung Stuff << */
        if($this->oxorder__oxbillsal->value=="MR"){
			$anrede = $this->translate( 'UNIV_SALUTATION_M' );
		}else{
			$anrede = $this->translate( 'UNIV_SALUTATION_F' );
		}
        if( oxRegistry::getConfig()->getRequestParameter('pdftype')=='mahnung1' ){
             $iTop -= 6;
               $oPdf->SetXY(14,  $iTop ); 
                $name = $anrede." ".ucfirst($this->oxorder__oxbillfname->value);
				$text = str_replace("[<NAME>]", $name, $this->cmstranslate('oxmahnung1'));
			//	$textheight = $oPdf->getStringHeight(180, $text );
				$oPdf->MultiCell(180, 60, $text, 0, 'L', 0); 
		 	 $iTop += 64;
		} 
		 if( oxRegistry::getConfig()->getRequestParameter('pdftype')=='mahnung2' ){
             $iTop -= 6;
               $oPdf->SetXY(14,  $iTop ); 
                $name = $anrede." ".ucfirst($this->oxorder__oxbillfname->value);
				$text = str_replace("[<NAME>]", $name, $this->cmstranslate('oxmahnung2'));
			//	$textheight = $oPdf->getStringHeight(180, $text );
				$oPdf->MultiCell(180, 60, $text, 0, 'L', 0); 
		 	 $iTop += 64;
		} 
    ...
    public function exportDeliveryNote($oPdf)
    {
        ...
        $oPdf->setFont($oPdfBlock->getFont(), 'B', 7);
        $oPdf->text(130, 50, $oShop->oxshops__oxcompany->getRawValue() . ' - ' . $oShop->oxshops__oxstreet->getRawValue() . ' - ' . $oShop->oxshops__oxzip->value . ' ' . $oShop->oxshops__oxcity->getRawValue());
        $oPdf->line(130, 51, 195, 51);
        
        // delivery address
        $oPdf->setFont($oPdfBlock->getFont(), '', 10);
        if ($this->oxorder__oxdelsal->value) {
            
            $oPdf->text($horizoffset, 59, $this->oxorder__oxdelcompany->getRawValue());
            $oPdf->text($horizoffset, 63, $sSal);
            $oPdf->text($horizoffset, 67, $this->oxorder__oxdellname->getRawValue() . ' ' . $this->oxorder__oxdelfname->getRawValue());
         
            $oPdf->text($horizoffset, 71, $this->oxorder__oxdelstreet->getRawValue() . ' ' . $this->oxorder__oxdelstreetnr->value);
            $oPdf->text($horizoffset, 75, $this->oxorder__oxdelzip->value . ' ' . $this->oxorder__oxdelcity->getRawValue());
            $oPdf->text($horizoffset, 79, $this->oxorder__oxdelcountry->getRawValue());
        } else {
       ...
       // $oPdf->text(15, 70, $sText);
        ...
        // order number
        $oPdf->setFont($oPdfBlock->getFont(), 'B', 12);
        $this->_setPdfTitle(  $oPdf, 108, $this->translate('ORDER_OVERVIEW_PDF_DELIVNOTE') );
       // $oPdf->text(15, 108, $this->translate('ORDER_OVERVIEW_PDF_DELIVNOTE') . ' ' . $this->oxorder__oxordernr->value."-".$this->oxorder__oxbillnr->value );
   ...
   protected function _setPdfTitle($oPdf, $iTop, $trans){
       
        //$oPdf->setFont($this->getFont(), 'B', 12);
        $oPdf->text(15, $iTop, $trans . ' ' . $this->oxorder__oxordernr->value."-".$this->oxorder__oxbillnr->value );
       
   }
	
	/** RDE
     * Returns translated string from cms admin pages
     *
     * @param string $sString string to translate
     *
     * @return string
     */
	public function cmstranslate( $soxid )
    {
				$oContent = oxnew('oxcontent'); 
			    $oContent->loadByIdent($soxid);
				$oContent->loadInLang( $this->_iSelectedLang , $oContent->oxcontents__oxid->value );
				
				return $oContent->oxcontents__oxcontent->value;
